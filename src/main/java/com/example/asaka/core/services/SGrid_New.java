...
public class SGrid_New {
  ...
  // Cr By: Saidazim
  public String replaceLabels(String jsonInput) {
    try {
      ObjectMapper objectMapper = new ObjectMapper();
      JsonNode jsonArray = objectMapper.readTree(jsonInput);

      for (JsonNode jsonNode : jsonArray) {
        if (jsonNode.has("label")) {
          String label = jsonNode.get("label").asText();
          String newLabel = getGridTranslation(label);
          ((ObjectNode) jsonNode).put("label", newLabel);
        }
      }
  ...
  // Cr By: Saidazim
  public String getGridTranslation(String label) {
    Connection conn = null;
    PreparedStatement ps = null;
    ResultSet res = null;
    String translation = "";
    try {
      conn = DB.con(hds);
      ps = conn.prepareStatement("select core_util.get_translated_text(?) as text from dual");
      ps.setString(1, label);
      res = ps.executeQuery();
      if (res.next()) {
        translation = res.getString("text");
      }
    } catch (Exception e) {
      e.printStackTrace();
    } finally {
